...
<?php
  ...
  if ($havePost) {
    ...
    $yearOk = is_numeric($year) && (strlen($year) == 4);
    ...
    if (!$yearOk) {
      $errors .= '<li>Enter a valid year in yyyy format</li>';
      if ($focusId == '') $focusId = '#year';
    }
    ...
    if ($errors != '') {
      ...
      if ($dbOk) {
        ...
        echo $title . ', created in ' . $year . '</div>';
...
<h3>Add Movie</h3>
<form id="addForm" name="addForm" action="movies.php" method="post" onsubmit="return validateMovies(this);">
  <fieldset>
    <div class="formData">
