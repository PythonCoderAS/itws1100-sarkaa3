...
<?php
  if ($dbOk) {
    ...
    $create_query = 'create table if not exists actors (actorid int unsigned primary key auto_increment, first_names varchar(100) not null, last_name varchar(100) not null, dob char(10) not null);';
    $db->query($create_query);
    $create_query = 'create table if not exists movie_actors (movieid int unsigned not null, actorid int unsigned not null, PRIMARY KEY(movieid, actorid), FOREIGN KEY (movieid) REFERENCES movies(movieid), FOREIGN KEY (actorid) REFERENCES actors(actorid));';
    $db->query($create_query);
